...
class modDisplayNewsHelper {
	...
	var $version = "DisplayNews by BK 2.1";
	var $css_type = null;
	var $target = "";
		...
		if ( $this->params->get('video')!="" ) {
			...
			if(preg_match("#{(".$this->grabTags.")}#s",$row->text)) {
				...
				if ( $this->params->get('video') == 1) {
					...
					$tmp_row->text= $text;
					$this->plgAllvideos->onContentPrepare('com_content.article', $tmp_row, $jw_allvideos_params );
					$text= $tmp_row->text;
				}
...
//   . ($this->params->get( 'set_author_id') <> "" ?                                            "\n  AND (a.created_by IN (".$this->params->get( 'set_author_id').") )" : '')
   ...
   . (($this->params->get( 'set_date_range')==2 and ($this->params->get( 'set_date_since' )!="")) ?"\n  AND (((SUBSTR(YEARWEEK(ADDDATE(".$db->Quote($now).", INTERVAL ".$this->tzoffset." HOUR)),5,2)) = (SUBSTR(YEARWEEK(ADDDATE(a.".$this->created.", INTERVAL ".$this->tzoffset." HOUR)),5,2)) ))" : '')
   
   . (($this->params->get( 'set_date_range')==3 and ($this->params->get( 'set_date_since' )!="")) ?"\n  AND (((SUBSTR(YEARWEEK(ADDDATE(".$db->Quote($now).", INTERVAL ".$this->tzoffset." HOUR)),1,4)) = (SUBSTR(YEARWEEK(ADDDATE(a.".$this->created.", INTERVAL ".$this->tzoffset." HOUR)),1,4)) ))" : '')
...
$this->tzoffset = ($datenow->toUnix() - $dbdatenow->toUnix()) / 3600 ;
	...
	if ( $this->params->get('avoid_shown')==3 ) {
		...
		if ( $this->params->get('video')!="" ) {
			require_once(dirname(__FILE__).DS.'..'.DS.'..'.DS.'plugins'.DS.'content'.DS.'jw_allvideos'.DS.'jw_allvideos'.DS.'includes'.DS.'helper.php');
			require(dirname(__FILE__).DS.'..'.DS.'..'.DS.'plugins'.DS.'content'.DS.'jw_allvideos'.DS.'jw_allvideos'.DS.'includes'.DS.'sources.php');

			if ( $this->params->get('video') ) {
				jimport('joomla.html.parameter');

				$jw_allvideos_plugin =& JPluginHelper::getPlugin('content',"jw_allvideos");
				// echo "DEBUG:".print_r($jw_allvideos_plugin, true );
				$jw_allvideos_params = new JParameter( $jw_allvideos_plugin->params );
				if ( $this->params->get('vwidth') ) {
					$jw_allvideos_params->set('vwidth', $this->params->get('vwidth'));
				}
				if ( $this->params->get('vheight') ) {
					$jw_allvideos_params->set('vheight', $this->params->get('vheight'));
				}
				if ( $this->params->get('awidth') ) {
					$jw_allvideos_params->set('awidth', $this->params->get('awidth'));
				}
				if ( $this->params->get('aheight') ) {
					$jw_allvideos_params->set('aheight', $this->params->get('aheight'));
				}
				
				if ( $this->params->get('autoplay') != "") {
					$jw_allvideos_params->set('autoplay', $this->params->get('autoplay'));
				}				
   ...
   $v = 0;
	...
	if ( $this->params->get('avoid_shown') ) {
      ...
      //---------------------------------------------------------------------
		...
		list( $text_out, $img_out, $video_out) = $this->text_out($row, $aroute);
		...
		$row_out = str_replace(array("%c","%S","%d","%t","%h","%a","%p","%v","%b","%i","%r","%m","%C"), 
								array($cat_out,$cat_desc_out,$date_out,$title_out,$hits_out, $author_out, $img_out, $video_out, $before_out, $text_out, $rate_out, $readmore_out, $jcomments_out), 
								$format);
                } else {
...
';
	...
	if ( $rows_out == '' and !$this->params->get('if_no_articles') ) {
		return;
	}
	...
	// End of Loop //
	if ($rows_out == '' and ($this->params->get('if_no_articles') == 2)) {
		$rows_out=$this->params->get('no_articles_message',"Oops!");
	}
	
	if (($rows_out != '') or ($this->params->get('if_no_articles') == 1) ) {
...

	{
		...
		if ( $this->params->get('use_module_template') == 1) {
