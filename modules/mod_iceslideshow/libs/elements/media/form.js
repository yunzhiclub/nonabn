...
// JavaScript Document\
 ...
 function removeFile( obj ){
	var parent = $$(obj).getParent("div").getParent("li");
	parent.getElement(".lof-enable").set("value", 0);
	parent.getElement(".lof-cols input").set("value", "");
	parent.fade("out");
	parent.setStyle("display","none");
 }
function addFile( ){
	var listFiles = $$("#module-sliders .panel .group-file");
	for( i= 0; i< listFiles.length;i++){
		item = listFiles[ i ];
		var enableField = item.getElement(".lof-enable");
		if(enableField){
			if(enableField.value == 0){
				item.fade("in");
				item.setStyle("display","block");
				enableField.set("value", 1);
				break;
			}
		}
	}
	/*
	listFiles.each( function(item,i){
		var enableField = item.getElement(".lof-enable");
		if(enableField){
			if(enableField.value == 0){
				item.fade("in");
	...
	var lis = document.getElements('#module-sliders ul.adminformlist li');
	 lis.each( function(li, index){
		...
		if(tmp)
		{
			...
			//showEnableFields(tmp.getProperty('title'));
			}
		}
	 });
	 function showEnableFields(name){
		if(name =="file"){
			var listFiles = $$("#module-sliders .panel .group-file");
			for( i= 0; i< listFiles.length;i++){
				item = listFiles[ i ];
