...
abstract class modIceSlideshowHelper
{
	/**
	 * get list articles
	 */
	public static function getList($params)
	{
		...
		{
			$cache =& JFactory::getCache('mod_iceslideshow');
			$cache->setCaching(true);
			$cache->setLifeTime($params->get('cache_time', 15) * 60);	
			return $cache->get(array('modIceSlideshowHelper' , 'getGroupObject'), array($params)); 
		}
		else
		{
	...
	public static function getGroupObject($params)
	{
		...
		{
			require_once($file);
			$className = 'IceSliderGroup'.ucfirst($group);
	...
	public function renderItem(&$row, $params, $layout='_item')
	{
		...
		$tPath 		= JPATH_BASE.DS.'templates'.DS.$mainframe->getTemplate().DS.'html'.DS.'mod_iceslideshow'.DS.$theme.DS.$layout.'.php';
		$bPath 		= JPATH_BASE.DS.'modules'.DS.'mod_iceslideshow'.DS.'themes'.DS.$theme.DS.$layout.'.php';
