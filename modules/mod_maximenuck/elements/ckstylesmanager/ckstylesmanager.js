...
function loadStylesCK(popup, fieldName) {
    ...
    /*slides = JSON.encode($$('.'+identifier));
    slides = slides.replace(/"/g,"|qq|");
    document.id(fieldName).value = slides;*/
    ...
    fields.each(function(field){
        ...
        if (fieldobj.getProperty('isradio')) {
            fieldobj.getParent().getElements('input[type=radio]').each(function(el) {
                // alert(el.value+" val"+fieldobj.value);
                // console.log('id:'+fieldobj.id+' val'+fieldobj.value);
                if (el.value == field['value']) {
                    el.setProperty('checked', 'checked');
                    if (el.getParent().hasClass('boutonRadio')) el.getParent().addClass('coche');
                    // console.log(el.id+' val'+field['value']+' active');
                }
                else {
                    el.removeProperty('checked');
                    if (el.getParent().hasClass('boutonRadio')) el.getParent().removeClass('coche');
                    // console.log(el.id+' desactive');
                }
            });
        }   
//alert(field['id']);
        fieldobj.value = field['value'];
        // if (fieldobj.hasClass('color')) fieldobj.fireEvent('change');
    });
    // alert(fields);
}
